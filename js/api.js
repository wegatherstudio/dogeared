...
================================================================ */
const DEFAULT_GRADIENT = ["#2A2A2E", "#1C1C1F", "#101012"]; // near-black, grayscale
const INK = "#FFFFFF";
...
function drawShareCard(canvas, opts) {
  ...
  const drawCoverPlaceholder = (cx, cy, cw, ch) => {
    const hue = GENRE_HUES[(opts.book?.g || [])[0]] || "#4A4A4E";
  ...
  const drawTextBlock = (titleY) => {
    ...
    // achievement badges — icons only, no labels; a subtle backdrop chip
    ...
    chosen.forEach((b, i) => {
      ...
      const paintArt = (img) => {
        ...
        if (img) {
          ...
          const hue = GENRE_HUES[(b.g || [])[0]] || "#4A4A4E";
...
function themeStopsFromGenre(book) {
  const hex = GENRE_HUES[(book?.g || [])[0]] || "#4A4A4E";
